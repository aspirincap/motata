...
from motata_cli.common.utils import filter_empty, json_compact, load_json_file, parse_json_option, parse_positive_int_str, validate_domain, validate_iso_datetime, validate_name, validate_non_empty
...
def _normalize_json_text(raw: Any, label: str, expected_type: type | tuple[type, ...] = dict) -> str | None:
    ...
    if isinstance(raw, str):
        value = parse_json_option(raw, label, expected_type)
        ...
        raise CliError(f"Invalid {label}: expected {expected_names}")
...
def _optional_positive_int_str(value: Any, label: str) -> str | None:
    return parse_positive_int_str(value, label) if value not in (None, "") else None
...
def parse_bid_constraints(raw: Any, label: str = "bid_constraints") -> dict[str, Any] | None:
    constraints = parse_json_option(raw, label, dict)
...
def validate_bid_strategy_constraints(
    ...
    *,
    ...
    if bid_strategy in BID_STRATEGIES_REQUIRING_BID_CAP:
        ...
        if required_value is None:
            raise CliError(f"{label_prefix}: {required_label} required for {bid_strategy}")
    if bid_strategy == BID_STRATEGY_MIN_ROAS:
        if not bid_constraints or bid_constraints.get("roas_average_floor") is None:
            raise CliError(
                f"{label_prefix}: bid_constraints.roas_average_floor required for {BID_STRATEGY_MIN_ROAS}"
            )


def normalize_targeting_spec(raw: Any, label: str = "targeting") -> dict[str, Any] | None:
    targeting = parse_json_option(raw, label, dict)
    ...
    if age_min is not None:
        ...
        if age_min < 13 or age_min > 65:
            raise CliError(f"{label}: age_min must be between 13 and 65")
        targeting["age_min"] = age_min
    if age_max is not None:
        age_max = int(age_max)
        if age_max < 13 or age_max > 65:
            raise CliError(f"{label}: age_max must be between 13 and 65")
        targeting["age_max"] = age_max
    if age_min is not None and age_max is not None and age_min > age_max:
        raise CliError(f"{label}: age_min cannot be greater than age_max")
    genders = targeting.get("genders")
    if genders is not None:
        if not isinstance(genders, list):
            raise CliError(f"{label}: genders must be a list")
        normalized_genders = []
        for gender in genders:
            parsed = int(gender)
            if parsed < 0 or parsed > 2:
                raise CliError(f"{label}: genders values must be 0, 1, or 2")
    ...
    if geo_locations is not None:
        if not isinstance(geo_locations, dict):
            raise CliError(f"{label}: geo_locations must be an object")
        countries = geo_locations.get("countries")
        if countries is not None:
            if not isinstance(countries, list):
                raise CliError(f"{label}: geo_locations.countries must be a list")
            normalized_countries = []
            for country in countries:
                code = str(country).strip().upper()
                if not re.fullmatch(r"[A-Z]{2}", code):
                    raise CliError(f"{label}: invalid country code {country!r}")
    ...
    if targeting_automation is None:
        ...
        if not isinstance(targeting_automation, dict):
            raise CliError(f"{label}: targeting_automation must be an object")
        advantage_audience = targeting_automation.get("advantage_audience")
        if advantage_audience not in (0, 1):
            raise CliError(f"{label}: targeting_automation.advantage_audience must be 0 or 1")
    return targeting


def normalize_promoted_object(raw: Any, label: str = "promoted_object") -> dict[str, Any] | None:
    promoted_object = parse_json_option(raw, label, dict)
...
def normalize_frequency_control_specs(raw: Any, label: str = "frequency_control_specs") -> list[dict[str, Any]] | None:
    value = parse_json_option(raw, label, list)
    if value is None:
        return None
    for item in value:
        if not isinstance(item, dict):
            raise CliError(f"{label}: each item must be an object")
    return value


def normalize_tracking_specs(raw: Any, label: str = "tracking_specs") -> list[dict[str, Any]] | None:
    specs = parse_json_option(raw, label, list)
    if specs is None:
        return None
    for item in specs:
        if not isinstance(item, dict):
        ...
        if not isinstance(action_types, list) or not all(isinstance(x, str) and x for x in action_types):
            raise CliError(f"{label}: each item requires 'action.type' as a non-empty string list")
        ...
        if fb_pixel is not None and (not isinstance(fb_pixel, list) or not all(isinstance(x, str) and x for x in fb_pixel)):
            raise CliError(f"{label}: fb_pixel must be a string list when provided")
    return specs
...
def build_campaign_payload(source: CampaignSpec | argparse.Namespace | dict[str, Any], *, update: bool = False) -> dict[str, Any]:
    ...
    _assign(payload, "name", validate_name(spec.name, "campaign name"))
    ...
    if spec.special_ad_categories:
        payload["special_ad_categories"] = json_compact(normalize_special_ad_categories(spec.special_ad_categories))
        ...
        payload["special_ad_categories"] = json_compact([])
    ...
    payload = filter_empty(payload)
    if not update:
        payload["name"] = validate_non_empty(payload.get("name"), "campaign name")
        payload["objective"] = validate_non_empty(payload.get("objective"), "objective")
    if payload.get("daily_budget") and payload.get("lifetime_budget"):
        raise CliError("campaign: cannot set both daily_budget and lifetime_budget")
    ...
    if bid_constraints is None and payload.get("bid_constraints") not in (None, ""):
        bid_constraints = parse_json_option(payload.get("bid_constraints"), "bid_constraints", dict)
    ...
    if update and not payload:
        raise CliError("No update fields provided. Pass at least one update option.")
    return payload
...
def build_adset_payload(source: AdSetSpec | argparse.Namespace | dict[str, Any], *, update: bool = False) -> dict[str, Any]:
    ...
    _assign(payload, "name", validate_name(spec.name, "ad set name"))
    ...
    _assign(payload, "bid_strategy", spec.bid_strategy)
    _assign(payload, "start_time", validate_iso_datetime(spec.start_time, "start_time"))
    _assign(payload, "end_time", validate_iso_datetime(spec.end_time, "end_time"))
    _assign(payload, "destination_type", spec.destination_type)
    _assign(payload, "promoted_object", json_compact(promoted_object) if promoted_object is not None else None)
    _assign(payload, "targeting", json_compact(targeting) if targeting is not None else None)
    _assign(
        payload,
        "frequency_control_specs",
        json_compact(frequency_control_specs) if frequency_control_specs is not None else None,
    )
    _assign(payload, "bid_constraints", json_compact(bid_constraints) if bid_constraints is not None else None)
    ...
    if not update:
        payload["campaign_id"] = validate_non_empty(payload.get("campaign_id"), "campaign_id")
        payload["name"] = validate_non_empty(payload.get("name"), "ad set name")
        payload["optimization_goal"] = validate_non_empty(payload.get("optimization_goal"), "optimization_goal")
        payload["billing_event"] = validate_non_empty(payload.get("billing_event"), "billing_event")

    if payload.get("daily_budget") and payload.get("lifetime_budget"):
        raise CliError("adset: cannot set both daily_budget and lifetime_budget")

    normalized_bid_constraints = (
        parse_json_option(payload.get("bid_constraints"), "bid_constraints", dict)
    ...
    if payload.get("start_time") and payload.get("end_time"):
        ...
        if end_dt <= start_dt:
            raise CliError("adset: end_time must be later than start_time")

    optimization_goal = payload.get("optimization_goal")
    promoted_object_payload = (
        parse_json_option(payload.get("promoted_object"), "promoted_object", dict)
...
def build_ad_payload(source: AdSpec | argparse.Namespace | dict[str, Any], *, update: bool = False) -> dict[str, Any]:
    ...
    _assign(payload, "name", validate_name(spec.name, "ad name"))
    ...
    if spec.creative_id is not None:
        payload["creative"] = json_compact({"creative_id": spec.creative_id})
    _assign(payload, "bid_amount", _optional_positive_int_str(spec.bid_amount, "bid_amount"))
    if spec.tracking_specs_json is not None:
        payload["tracking_specs"] = json_compact(normalize_tracking_specs(spec.tracking_specs_json))
    _assign(payload, "conversion_domain", validate_domain(spec.conversion_domain, "conversion_domain"))
    ...
    if not update:
        payload["name"] = validate_non_empty(payload.get("name"), "ad name")
        payload["adset_id"] = validate_non_empty(payload.get("adset_id"), "adset_id")
    if update and not payload:
        raise CliError("No update fields provided. Pass at least one update option.")
    return payload
...
def build_story_spec(source: CreativeSpec | argparse.Namespace | dict[str, Any]) -> dict[str, Any]:
    ...
    if spec.object_story_spec:
        return parse_json_option(spec.object_story_spec, "object_story_spec", dict) or {}
    if not spec.page_id:
        raise CliError("Missing --page-id or --object-story-spec")
    ...
    if not spec.image_hash or not spec.link:
        raise CliError("Image creative mode requires --page-id, --image-hash and --link.")
...
def build_creative_payload(
    ...
    *,
    ...
    if not update:
        payload["name"] = validate_non_empty(payload.get("name"), "creative name")
    if update and not payload:
        raise CliError("No update fields provided. Pass at least one update option.")
    return payload
